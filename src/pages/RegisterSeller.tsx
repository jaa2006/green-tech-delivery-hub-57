...
import { User, LogIn, ArrowRight, Loader2, ArrowLeft, Store } from "lucide-react";
...
const RegisterSeller = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use AuthContext signup method for sellers
      await signup(email, password, name, 'seller', undefined, {
        storeName,
        storeDescription
      });

      toast({
        title: "Akun seller berhasil dibuat",
        description: "Selamat datang di Habisin, Seller!",
      });
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      if (!storeName) {
        toast({
          title: "Error",
          description: "Mohon isi nama toko terlebih dahulu",
          variant: "destructive",
        });
      ...
      // Save seller data directly to sellers collection
      const sellerData = {
        uid: user.uid,
      ...
      console.log('Saving Google seller data to sellers collection:', sellerData);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#07595A] to-black flex flex-col">
      ...
      <div className="bg-[#07595A] rounded-b-3xl h-24 flex items-center justify-center relative mb-8">
        <Link to="/login-seller" className="absolute left-6 top-1/2 -translate-y-1/2">
          <ArrowLeft className="h-6 w-6 text-white" />
        </Link>
        <h1 className="text-white text-2xl font-bold">Daftar Seller</h1>
      </div>
        ...
        <div className="max-w-md w-full">
          <div className="bg-gradient-to-b from-gray-800 to-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="storeName" className="text-white">Nama Toko</Label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <Label htmlFor="storeDescription" className="text-white">Deskripsi Toko (Opsional)</Label>
                <Input
                  id="storeDescription"
                  type="text"
