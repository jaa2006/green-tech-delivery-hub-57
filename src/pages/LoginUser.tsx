...
const LoginUser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check user role only in users collection
      console.log('Checking users collection for UID:', user.uid);
      const userDoc = await getDoc(doc(db, "users", user.uid));
      
      if (!userDoc.exists()) {
        showError("Akun user tidak ditemukan");
        setIsLoading(false);
        return;
      }
      ...
      if (userData.role !== "user") {
        showError("Akun ini bukan akun user");
        setIsLoading(false);
        return;
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Check if user exists in users collection only
      const userDoc = await getDoc(doc(db, "users", user.uid));
      ...
      if (!userDoc.exists()) {
        ...
        console.log('Creating new user in users collection:', userData);
