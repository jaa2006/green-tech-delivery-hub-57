...
const RegisterUser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use AuthContext signup method for users
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        errorMessage = "Quota Firebase habis, coba lagi nanti";
      }
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      console.log('✅ Google signup successful, UID:', user.uid);

      // Save user data directly to users collection
      const userData = {
      ...
      console.log('Saving Google user data to users collection...');
      console.log('User data:', userData);
      
      const userDocRef = doc(db, "users", user.uid);
      await setDoc(userDocRef, userData);
      
      console.log('✅ Google user data saved to users collection');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#07595A] to-black flex flex-col">
      ...
      <div className="bg-[#07595A] rounded-b-3xl h-24 flex items-center justify-center relative mb-8">
        ...
        <h1 className="text-white text-2xl font-bold">Daftar User</h1>
      </div>
